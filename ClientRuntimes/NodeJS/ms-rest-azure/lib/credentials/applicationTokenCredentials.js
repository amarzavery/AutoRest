    ...
    if (err) {
      if (err.message.match(/.*while removing the expired token for service principal.*/i) !== null) {
        ...
        self.context.acquireTokenWithClientCredentials(self.environment.activeDirectoryResourceId, self.clientId, self.secret, function (err, tokenResponse) {
          if (err) {
            //Since an error occurred while getting the token with the current secret, make sure that the service principal key is removed if present in the token cache.
            var query = {
              servicePrincipalId: self.clientId,
              servicePrincipalTenant: self.domain
            };
            _removeInvalidEntry.call(self, query, function () {
